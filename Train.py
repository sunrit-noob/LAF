...
'''
def testHypothesis(trained_model, retrained_model, test_loader, output_label = None, test_label = 0, labelAgnostic=False):
    trained_model = trained_model.cuda()
    ...
    with torch.no_grad():
        ...
        for i, (x,y) in enumerate(test_loader):
            ...
            predicted_set_original = torch.from_numpy(np.argsort(-y_out.numpy()), dtype=torch.float32)
            predicted_set_retrained = torch.from_numpy(np.argsort(-y_out_retrained.numpy()), dtype=torch.float32)
            if(labelAgnostic):
